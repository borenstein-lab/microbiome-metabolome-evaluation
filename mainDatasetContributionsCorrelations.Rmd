...
fake_mets_melt[,niceLab2:=ifelse(grepl("meso-2;6", niceLab), "meso-2;6-diamino\nheptanedioate", as.character(niceLab))]
met_dists = ggplot(fake_mets_melt[medium %in% path_key[!is.na(Class2),unique(medium)]], aes(x=value)) + geom_histogram(bins=15) + facet_wrap(~niceLab2, scales = "free", ncol=5)+ theme(strip.background = element_blank()) + xlab("Metabolite concentration") + ylab("Sample count") + theme(axis.text = element_text(size=5), axis.title = element_text(size=9), strip.text = element_text(size = 8))+ scale_y_continuous(expand = c(0,0))
...
fake_mets_melt[,mediaMet:=ifelse(medium %in% media[,medium],1,0)]
...
met_summary = fake_mets_melt[,list(mean(value,na.rm=T), var(value,na.rm=T), sd(value, na.rm=T)/mean(value, na.rm=T)),by=list(niceLab,medium)]
...
shap_contribs = merge(shap_contribs, met_summary, by.x = c("compound","niceLab"), by.y = c("medium","niceLab"), all.x=T, all.y=F)
...
fig_s5 = plot_grid(met_dist_mag,plot_grid(num_contribs_tax, num_players_tax + guides(fill = F), nrow = 1, rel_widths = c(1.25, 1), align = "h", axis = "tb"), nrow = 2, labels = c("A", "B"), rel_heights = c(1,1))
...
```{r, echo=F, message=F, warning=F}
setnames(fake_mets_melt, "medium", "compound")
...
real_shap2 = fread("~/GS_server/PROJECTS/MORE_METABOLOMICS_CN/RESULTS/shapValAGORA/AllShapContribs3.txt")
...
all_shap_iters = fread("~/GS_server/PROJECTS/MORE_METABOLOMICS_CN/RESULTS/shapValAGORA/AllCumulMetVars.txt")
