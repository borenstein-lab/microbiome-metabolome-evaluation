...
brite_files = list.files("../../KEGGfiles/KEGG_2018/", pattern = "br.*.keg")[1:3]
...
dictionary_sub = get_hmp_dictionary(dictionary_file, brite_files = brite_files, mets_melt = fake_mets_melt)
...
prev_plot = ggplot(num_contribs_by_prevalence[NumContribs >= 1], aes(x=PrevalenceCat, y = Specificity, color = phylum)) + geom_beeswarm(priority = "random", cex = 2) + xlab("Number of samples present") + ylab("Species-level specificity") + ylim(0.5, 1.05) + scale_color_manual(values = phylum_colors, name = "") + theme(axis.ticks.x = element_blank())
...
fig_sx_hmp = plot_grid(pcoa_compare_plot, met_pca_plot, prev_plot + theme(legend.text = element_text(size = 11)), met_category_plot + theme(legend.text = element_text(size = 10), axis.text = element_text(size = 9), axis.title.y = element_text(size = 10), legend.position = "bottom"), nrow = 2, labels = c("A", "B", "C", "D"), rel_widths = c(1.25, 1))
save_plot(fig_sx_hmp, file = "figureSx_hmp.png", base_width = 8, base_height = 7)
